...
router.get('/stats/summary', requirePermission(Permission.VIEW_SHIPMENTS), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const stats = {
      ...
      byStatus: {
        draft: shipments?.filter((s: any) => s.status === 'DRAFT').length || 0,
        pending: shipments?.filter((s: any) => s.status === 'PENDING').length || 0,
        booked: shipments?.filter((s: any) => s.status === 'BOOKED').length || 0,
        inTransit: shipments?.filter((s: any) => s.status === 'IN_TRANSIT').length || 0,
        arrived: shipments?.filter((s: any) => s.status === 'ARRIVED').length || 0,
        delivered: shipments?.filter((s: any) => s.status === 'DELIVERED').length || 0,
        exception: shipments?.filter((s: any) => s.status === 'EXCEPTION').length || 0,
        cancelled: shipments?.filter((s: any) => s.status === 'CANCELLED').length || 0,
      },
      byServiceType: {
        seaFcl: shipments?.filter((s: any) => s.service_type === 'SEA_FCL').length || 0,
        seaLcl: shipments?.filter((s: any) => s.service_type === 'SEA_LCL').length || 0,
        air: shipments?.filter((s: any) => s.service_type === 'AIR').length || 0,
        road: shipments?.filter((s: any) => s.service_type === 'ROAD').length || 0,
        rail: shipments?.filter((s: any) => s.service_type === 'RAIL').length || 0,
        multimodal: shipments?.filter((s: any) => s.service_type === 'MULTIMODAL').length || 0,
      },
    };
